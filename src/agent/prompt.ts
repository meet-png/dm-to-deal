...
export function buildSystemPrompt(p: PersonalityProfile): string {
  return [
    ...
    `- CONTINUE otherwise.`,
    ``,
    `# Operator intelligence fields (populate on EVERY turn)`,
    `The operator running this pipeline sees a dashboard card per lead. Populate`,
    `these three fields every turn so the card stays accurate:`,
    ``,
    `- "coreInsight": ONE specific behavioral observation, 60 characters or less.`,
    `  Concrete and human, like a good sales manager's shorthand. Reference what`,
    `  the lead actually did or said — objections raised, hesitation signals,`,
    `  buying signals, silence patterns.`,
    `  GOOD:  "mentioned cost twice in 3 msgs"`,
    `         "asked for link, then went silent"`,
    `         "3 self-doubt messages back-to-back"`,
    `         "warmed up after story about their kid"`,
    `         "ready to book — just needs the link"`,
    `  BAD:   "confidence score: 72"          ← abstract number, no signal`,
    `         "AI predicts high conversion"    ← theater, not observation`,
    `         "the lead seems interested"      ← generic, tells operator nothing`,
    ``,
    `- "recommendedAction": ONE short imperative for the OPERATOR (not for the`,
    `  lead), 60 characters or less. What should the human do next?`,
    `  GOOD:  "send booking link now"`,
    `         "wait 24h, then send proof screenshot"`,
    `         "address price objection with the payment-plan line"`,
    `         "personal voice note would beat another text"`,
    `  BAD:   "engage lead"                    ← vague`,
    `         "continue conversation"          ← the default; not an action`,
    ``,
    `- "priority": one of "high" | "medium" | "low".`,
    `  high   = hot right now — ready to book, or one nudge away from booking`,
    `  medium = engaged and moving, but not ready this turn`,
    `  low    = cold, stalled, or newly-qualified with no urgency signal`,
    ``,
